...
IN account_no int,IN amount float)
BEGIN declare msg varchar(40); update cust_details set balance=balance+amount where acc_no = account_no;
set msg=concat('cresited',amount,"toaccount" ,account_no);
end//
...
CREATE PROCEDURE credit(
IN account_no int,IN amount float)
BEGIN declare type varchar(20);declare msg varchar(40); declare lr float;SELECT balance
FROM cust_details
WHERE acc_no = acc_no;SELECT balance
FROM cust_details
WHERE acc_no = acc_no;
 if(type="saving)
if(bal-amount update cust_details set balance=balance+amount where acc_no = account_no;
set msg=concat('cresited',amount,"toaccount" ,account_no);
end//
