...
BEGIN declare type varchar(20);declare msg varchar(50);declare bal float;
...
set msg=concat('debited',amount,"fromaccount" ,account_no);
else set
msg=CONCAT("CANNOT DEBIT. YOU DONT HAVE ENOUGH BALANCE");
END IF;
END IF;
 ...
 update cust_details set balance=balance-amount where acc_no = account_no;
set msg=concat('debited',amount,"fromaccount" ,account_no);
else set msg=CONCAT("CANNOT DEBIT. YOU DONT HAVE ENOUGH BALANCE");
end if;
end if;
...
insert into cust_details values(110,'CURRENT','rakeeb','dana rafa kalslsdk',5000);
DELIMITER //
create PROCEDURE Balance(account_no int)
begin
declare bal float;
declare msg varchar(40);
if(exists(select acc_no from cust_details WHERE acc_no = account_no))then
select balance into bal from cust_details WHERE acc_no = account_no;
set msg=concat(account_no,"has",bal);
else set msg=CONCAT("invalid account");
end if;
SELECT msg;
end//
